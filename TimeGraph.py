...
def drawBeanPlot(bt, bf, ba):
    ...
    plt.xlabel('')
    plt.ylabel('')
    plt.savefig('Fig/Slope_SkEE_SF.pdf', bbox_inches='tight')

    plt.show()

def save_slope(filename, t_b, f_b, a_b):
    if os.path.exists("Stats/Slope_suport_fraction.csv") == 0:
        f=open("Stats/Slope_suport_fraction.csv", "w")
        f.write('Filename,Time_slope,Flip_slope,ARI_Slope\n')
        f.close()

    f=open("Stats/Slope_suport_fraction.csv", "a")

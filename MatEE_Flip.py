...
def ee(filename, optSettings):
    ...
    Default
    '''
    runEE(filename, X, parameters_default, runs, 'Default', time[0][0])

    '''
    Optimal
    '''
    runEE(filename, X, optSettings, runs, "Optimal", time[0][1])
    '''
    Fast
    '''
    runEE(filename, X, parameters_fast, runs, 'Fast', time[0][2])
...
def drawGraphs(filename, labels, runs, mode):
    ...
    plt.savefig("FlipFig/"+filename+"_MatEE_"+mode+"_NormDist.pdf", bbox_inches="tight", pad_inches=0)
    plt.show()
    ...
    plt.savefig("FlipFig/"+filename+"_MatEE_"+mode+"_Count.pdf", bbox_inches="tight", pad_inches=0)
    plt.show()
...
if __name__ == '__main__':
    ...
    if os.path.exists("Stats/MatEE.csv"):
        df = pd.read_csv("Stats/MatEE.csv")
